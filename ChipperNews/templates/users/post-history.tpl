    ...
    <div class=allcontent>
		...
		<span class="caret caret-reversed"></span> 
         ...
         {foreach $comments as $comment}
            {$article = fetchArticle($comment.comment_id)}
            <div class="container comment-snip" id="">
                <div class="thumbcontent">
                    <div class="container image">
                        <img src="{$BASE_URL}images\articles\{$article.image}" alt="..." style=" width: 100%; height: auto;">
                    </div>
                    <h2 id="headline"><a href=# style="color:black" id="articleAnchor">{$article.title}</a></h3>
                </div>
                <div class="comment">
                    <div class="mycomment">
                        <h6>By <a style="color:black; font-style:italic">{$user[0].username}</a> {$comment.posted_date}</h6>
                        <p>{$comment.content}</p>
                        {$votes = getVotes($comment.comment_id)}
                        {if $votes >= 0}
                            <h5>Votes: <span class="posrating" style="font-weight:bold;color:#357266">+{$votes}</span></h5>
                        {else}
                            <h5>Votes: <span class="posrating" style="font-weight:bold;color:#f11066">-{$votes}</span></h5>
                        {/if}
                    </div>
                </div>
            </div>
