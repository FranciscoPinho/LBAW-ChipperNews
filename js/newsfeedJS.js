...
function fullStory(snipID) {
	...
	switch (snipID) {
		case '1':
			$(document.getElementById(`article-body-${snipID}`)).append(`<p>Recently, the number of processing cores per CPU has rapidly scaled up, but they quickly reached the point of diminishing returns. Now, getting the most processing power for each Watt seems to be the key measure of performance.</p>
		...
		case '2':
			$(document.getElementById(`article-body-${snipID}`)).append(`<p>Let's have a look at a few different benchmarks people have done.</p>
		...
		case '3':
			$(document.getElementById(`article-body-${snipID}`)).append(`<ul>
